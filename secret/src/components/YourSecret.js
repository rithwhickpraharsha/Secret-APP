...
async function Remove(){
    try{
      ...
      await axios.post('https://secret-app-gamma.vercel.app/secret/remove',{token:token});
    }
    catch(e){
        console.log(e);
...
useEffect(()=>{
    async function show(){
        const token= localStorage.getItem('secret-app');
        if(!token)navigate("/");
        const data = await axios.post('https://secret-app-gamma.vercel.app/secret/show/user',{token:token});
